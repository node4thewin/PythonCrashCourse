...
# To begin, we need a file with a few lines of text in it. Let's start with a file that contains 'pi' to 30 decimal places, with 10 decimale places per line:

with open('pi_digits.txt') as file_object:
  contents = file_object.read()
print(contents)

# The first line of this program has a lot going on. Let's start by looking at the 'open()' function. To do any work with a file, even just printing its contents, you first need to 'open' the file to access it. The 'open()' function needs one argument: the name of the file you want to open. Python looks for this file in the directory where the program that's currently being executed is stored. In this example, 'file_reader.py' is currently running, so Python looks for 'pi_digits.txt' in the directory where 'file_reader.py' is stored. The open() function returns an object representing the file. Here, 'open('pi_digits.txt') returns an object representing 'pi_digits.txt'. Python assings this object to 'file_object', which we'll work with later in this program.
