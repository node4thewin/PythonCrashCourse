...
# The 'text' attribute is a 'CharField'--a piece of data that's made up of characters, or text (line 15). You use 'CharField' when you want to store a small amount of text, such as a name, a title, or a city. When we define a 'CharField' attribute, we have to tell Django how much space it should reserve in the database. Here we give it a 'max_length' of 200 characters, which should be enough to hold most topic names.

# The 'date_added' attribute is a 'DateTimeField'--a piece of data taht will record a date and time (line 16). We pass the argument 'auto_now_add=True', which tells Django to automatically set this attribute to the current date and time whenever the user creates a new topic.

# NOTE - To see the different kinds of fields you can use in a model, see the 'Django Model Field Reference' at https://docs.djangoproject.com/en/3.1/ref/models/fields. You won't need all the information right now, but it will be extremely useful when you're developing your own apps.

# We tell Django which attribute to use by default when it displays infromation about topic. Django calls a __str__() method to display a simple representation of a model. Here we've written a __str__() method that returns the string stored in the 'text' attribute (line 18).